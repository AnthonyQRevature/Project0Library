...
(
    SELECT book_count.title, book_count.isbn, book_count.num_copies AS num_copies, agg_genres.genre
    FROM
    (
        SELECT title, titles.isbn, COUNT(unborrowed.book_id) AS num_copies
        FROM unborrowed INNER JOIN titles USING (isbn)
    ...
    (
        ...
        GROUP BY titles.isbn
    ) AS agg_genres USING (isbn)
    WHERE book_count.num_copies > 0
    ORDER BY title ASC
);
